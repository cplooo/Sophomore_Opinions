...
####### 選擇院系
###### 選擇 院 or 系:
院_系 = st.text_input('以學系查詢請輸入 0, 以學院查詢請輸入 1  (說明: (i).以學系查詢時同時呈現學院及全校資料. (ii)可以選擇比較單位): ', value='0')
if 院_系 == '0':
...
with st.expander("1-1 選擇目前就讀科系的理由 (多選):"):
...
with st.expander("1-3 您的大一專業基礎課程學習情況，學習不良的課程 (多選):"):
    #df_sophomore.columns
    # df_sophomore.iloc[:,8] ## 2.您的大一專業基礎課程學習情況，學習良好的課程名稱請以「,」隔開，如無，請填「無」
    column_index = 9
    ...
    ##### 更改 series 的 index 欄位名稱
    # counts.index.names = ['科系', '學習良好課程']
    counts.index.names = ['學習不良課程']
    ##### 将 MultiIndex Series 'counts' 转换为DataFrame
    #### 將 兩個index 變columns
    counts_df = counts.reset_index()
    #### 將新的兩個columns 重新命名
    # counts_df.columns = ['科系', '學習良好課程', '人數']
    counts_df.columns = ['學習不良課程', '人數']
    # print(counts_df)
...
with st.expander("1-5 學習及生活費(書籍、住宿、交通、伙食等開銷) 主要來源:"):
    # df_sophomore.columns
